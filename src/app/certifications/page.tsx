    ...
    <main>
      ...
      <section className="bg-white py-20 sm:py-24 lg:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="eyebrow text-slate-600">Core credentials</p>
            <h2 className="mt-4 font-display text-4xl font-extrabold tracking-tight text-slate-950 sm:text-5xl">Standards that support buyer confidence.</h2>
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {CERTIFICATIONS.map((cert, index) => {
              ...
              return (
                <motion.article key={cert.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className="rounded-[1.7rem] border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:border-stone-200 hover:shadow-xl">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-900 text-amber-300"><Icon className="h-6 w-6" /></div>
                  <p className="mt-6 text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-600">{cert.category}</p>
                  <h3 className="mt-2 font-display text-xl font-extrabold text-slate-950">{cert.title}</h3>
      ...
      <section className="bg-[#f4f0e6] py-20 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:px-8">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="overflow-hidden rounded-[2rem] border border-black/5 bg-white p-3 shadow-xl shadow-slate-950/10">
          ...
          <div>
            <p className="eyebrow text-slate-600">Official attestations</p>
            <h2 className="mt-4 font-display text-4xl font-extrabold tracking-tight text-slate-950 sm:text-5xl">Organic-input compliance across key frameworks.</h2>
      ...
      <section className="bg-slate-950 py-20 text-white sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="eyebrow text-amber-300">Beyond the certificate</p>
            ...
            {[
              ...
              return (
                <article key={title as string} className="rounded-[1.7rem] border border-white/10 bg-white/[0.055] p-7 backdrop-blur-sm">
                  <QualityIcon className="h-7 w-7 text-amber-300" />
          ...
          <div className="mt-12 text-center">
            <Link href="/#contact" className="inline-flex items-center gap-2 rounded-full bg-amber-300 px-7 py-4 text-sm font-extrabold text-slate-950 transition-colors hover:bg-amber-200">Request certification information <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
    </main>
